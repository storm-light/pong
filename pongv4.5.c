...
/* NOTES
   ...
   Most importantly stay organized. Try to put things in function things.

   Perhaps hotkeys to add special effects:
   Ghost v
   Speed Boost v
   Change direction (with lowered speed) v
   Omnicontrol v
...
int main(void)
{
    ...
    SDL_Window* win = SDL_CreateWindow("PONG ON STEROIDS lol",
    ...
    SDL_Texture * image = renderText("POINTS:", "Spoopies.ttf", color, 15, rend);
    SDL_Texture * image1 = renderText("POINTS:", "Spoopies.ttf", color1, 15, rend);
    if (image == 0 || image1 == 0)
    {
    ...
    d.h = iH;
    /* loading textures */
    SDL_Texture* tex = loadTexture(rend, "blue.png");
    SDL_Texture* tex1 = loadTexture(rend, "blue.png");
    SDL_Texture* text = loadTexture(rend, "blue.png"); 
    SDL_Texture* backg = loadTexture(rend, "white.jpg");
    ...
    SDL_SetTextureBlendMode(text, blend);
    Uint8 a = 255;
    ...
    float ballXV = SPEED - (rand() % 50);
    float ballYV = SPEED - (rand() % 50);
    ...
    while (!closeME)
    {
        ...
        if (alt && p > 19)
        {
            if (up) ballYV -= BALL_ACC;
            if (down) ballYV += BALL_ACC;
            if (left) ballXV -= BALL_ACC;
            if (right) ballXV += BALL_ACC;
        }

        if (alt1 && p1 > 19)
        {
            if (up1) ballYV -= BALL_ACC;
            if (down1) ballYV += BALL_ACC;
            if (left1) ballXV -= BALL_ACC;
            if (right1) ballXV += BALL_ACC;
        }
        ...
        if (ballX <= dest.w + 3)
        {
            ...
            else
            {
                ...
                ballXV = SPEED - (rand() % 50);
                ballYV = SPEED - (rand() % 50);
                if (ballXV < 0) ballXV = -ballXV;
                if (rand() & 0x1) ballYV = - ballYV;
                dest.h -= 40;
                if (dest.h == 0)
                {
        ...
        SDL_Texture * num = renderText(snum, "Spoopies.ttf", black, 15, rend);
        ...
        SDL_Texture * num1 = renderText(snum1, "Spoopies.ttf", black, 15, rend);
    ...
    SDL_Texture * score = renderText("SCORE: ", "Spoopies.ttf", color, 15, rend);
    ...
    if (p > p1)
    {
        ...
        SDL_Texture * pOne = renderText("PLAYER 1 WINS", "Spoopies.ttf", color, 50, rend);
    ...
    else if (p1 > p)
    {
        SDL_SetRenderDrawColor(rend, 142, 190, 0, 255);  
        SDL_RenderFillRect(rend, NULL);
        SDL_Texture * pTwo = renderText("PLAYER 2 WINS", "Spoopies.ttf", color, 50, rend);
    ...
    else if (p1 == p)
    {
        ...
        SDL_RenderFillRect(rend, NULL);
        SDL_Texture * Tie = renderText("TIE", "Spoopies.ttf", color, 70, rend);
